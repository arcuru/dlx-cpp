...
DLX::DLX(vector<vector<size_t>> definition, size_t req_columns)
  ...
  for (auto &v : nodes_) link_columns(v, col_headers);
...
void DLX::require_row(size_t row) {
  ...
  while (ptr->column != column && ptr != matrix) ptr = ptr->right;

  if (ptr == matrix) throw std::invalid_argument("Incompatible requirements.");

  auto rp = ptr->down;
  while (rp->row != row && rp != ptr) rp = rp->down;

  if (rp == ptr) throw std::invalid_argument("Incompatible requirements.");
